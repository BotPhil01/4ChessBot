...
int main() {
    array<types::Move, 20> a_moves;
    for (int i = 0; i < 5; i++) {
        cout << i << endl;
        Board b = Board();
        Engine e = Engine(b, PieceColour::RED, 12);
        a_moves[i] = e.chooseNextMove();
    } 
    cout << a_moves[19].totalMoves << endl;
}
...
//         meter.measure([&e] { return e.chooseNextMove(); });
//     };
//     BENCHMARK_ADVANCED("Choose 20")(Catch::Benchmark::Chronometer meter) {
//         Board b = Board();
//         Engine e = Engine(b, PieceColour::RED, 20);
//         meter.measure([&e] { return e.chooseNextMove(); });
//     };
